...
## Verrou de rôle (en place)

La coquille lit `members.role` à la connexion et le passe au composant. Un
compte `terrain` ne rend **que** l'app terrain : le launcher est court-circuité
par une garde de routage, le bouton Bureau est masqué, et aucun bouton interne
ne permet d'en sortir. Un compte `bureau` garde l'accès aux deux espaces.

**Limite connue (v1, assumée).** Le verrou est au niveau interface. Les données
elles-mêmes (`app_kv`, clé `roovers-team` incluant les taux horaires) restent
lisibles par tout membre de l'organisation via la RLS actuelle : un équipier
techniquement outillé pourrait interroger l'API Supabase directement avec sa
session. Acceptable pour une équipe interne de confiance ; pour un cloisonnement
réel, il faudra en v2 déplacer les champs sensibles (salaires, marges) vers une
table dédiée avec une policy `role = 'bureau'`.
