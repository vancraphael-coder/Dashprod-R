...
function Root() {
  ...
  const [member, setMember] = useState(undefined);   // undefined=?, null=non membre, {org_id, role}=ok
  ...
  // ET on lit le rôle (bureau/terrain) — une seule requête, filtrée par RLS.
  useEffect(() => {
  ...
  if (member === undefined) return <Center>Vérification du compte…</Center>;
  if (member === null)
    return (
      <Center>
