...
1. ~~**Verrou de rôle**~~ — **fait** : la coquille lit `members.role`, une garde
   de routage rend l'app terrain seule pour les comptes `terrain` (voir README,
   « Verrou de rôle »). Reste en v2 : cloisonnement au niveau données
   (salaires/marges hors de `app_kv`, policy `role = 'bureau'`).
