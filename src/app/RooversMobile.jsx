...
/* Normalisation : un dossier chargé du stockage peut venir d'une version
   antérieure de l'app (champs absents). On le fond sur les défauts pour
   garantir la forme — c'est CE qui évite les crashs de rendu. */
function normDossier(x){
  if(!x||typeof x!=="object") return newDossier();
  const d={...newDossier(),...x};
  if(!d.id) d.id=uid();
  if(!Array.isArray(d.inventaire)) d.inventaire=[];
  if(!d.emballage||typeof d.emballage!=="object"||Array.isArray(d.emballage)) d.emballage={};
  if(!Array.isArray(d.camions)) d.camions=[];
  if(!Array.isArray(d.equipe)) d.equipe=[];
  return d;
}
/* Fusion à la sauvegarde : bureau et terrain écrivent la même liste.
   Au lieu d'écraser (dernier écrit gagne), on relit le stockage et on garde,
   dossier par dossier, la version la plus récente (horodatage maj).
   Les suppressions laissent une pierre tombale pour ne pas ressusciter. */
function mergeDossiers(localArr,storedArr,tombs){
  const M=new Map();
  for(const x of (Array.isArray(storedArr)?storedArr:[])) if(x&&x.id) M.set(x.id,x);
  for(const x of (Array.isArray(localArr)?localArr:[])) if(x&&x.id){ const o=M.get(x.id); if(!o||num(x.maj||0)>=num(o.maj||0)) M.set(x.id,x); }
  for(const id in (tombs||{})){ const t=tombs[id]; const o=M.get(id); if(!o||num(o.maj||0)<=t) M.set(id,{id,deleted:true,maj:t}); }
  return [...M.values()];
}
const SEED_TEAM=[
...
export default function App(){
  ...
  useEffect(()=>{(async()=>{
    try{ if(window.storage){
      try{const r=await window.storage.get("roovers-dossiers"); if(r&&r.value){const a=JSON.parse(r.value); if(Array.isArray(a)&&a.length){const live=a.filter(x=>!(x&&x.deleted)).map(normDossier); if(live.length) setDossiers(live);}}}catch(e){}
  ...
  useEffect(()=>{ if(!loaded) return; clearTimeout(saveT.current);
    saveT.current=setTimeout(async()=>{ try{ if(window.storage){ let _md=dossiers; try{ const r0=await window.storage.get("roovers-dossiers"); if(r0&&r0.value){ const st=JSON.parse(r0.value); if(Array.isArray(st)) _md=mergeDossiers(dossiers,st,tombRef.current); } }catch(_){ } await window.storage.set("roovers-dossiers",JSON.stringify(_md),false); await window.storage.set("roovers-team",JSON.stringify(team),false); await window.storage.set("roovers-trucks",JSON.stringify(trucks),false); await window.storage.set("roovers-crm",JSON.stringify(crm),false); await window.storage.set("roovers-settings",JSON.stringify(settings),false); await window.storage.set("roovers-clients",JSON.stringify(clients),false);} }catch(e){ setStorageOK(false); } },700);
  ...
  const up=(patch)=>setDossiers(ds=>ds.map(x=>x.id===selId?{...x,...patch,maj:Date.now()}:x));
  ...
  const confirmDossier=(id)=>{setDossiers(ds=>ds.map(x=>x.id===id?{...x,pending:false,maj:Date.now()}:x)); show("Dossier confirmé");};
  ...
  const del=(id)=>{tombRef.current[id]=Date.now(); setDossiers(ds=>ds.filter(x=>x.id!==id));setView("list");show("Dossier supprimé");};
  ...
  const terrainLogin=()=>(
    <div style={{minHeight:"100vh",maxWidth:480,margin:"0 auto",background:C.bg,borderLeft:`1px solid ${C.border}`,borderRight:`1px solid ${C.border}`,padding:"44px 22px"}}>
      ...
      <button onClick={()=>setAppMode("launch")} style={{marginTop:22,padding:"12px",width:"100%",background:"none",border:"none",color:C.faint,fontSize:12.5,fontFamily:FS,cursor:"pointer"}}>← Retour à l'accueil</button>
    </div>
  );
  ...
  const launcher=()=>(
    <div style={{minHeight:"100vh",maxWidth:480,margin:"0 auto",background:`linear-gradient(180deg,${C.navy},#0b1220)`,fontFamily:FS,color:"#fff",display:"flex",flexDirection:"column",justifyContent:"center",padding:"30px 22px",borderLeft:`1px solid ${C.border}`,borderRight:`1px solid ${C.border}`}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <button onClick={()=>{setAppMode("bureau");setView("list");setSelId(null);}} style={{display:"flex",alignItems:"center",gap:15,padding:"20px",borderRadius:18,border:"none",background:"#fff",cursor:"pointer",textAlign:"left"}}>
  ...
  if(appMode==="terrain") return <ErrorBoundary full onReset={()=>{setTScreen("jobs");setCrmClient(null);}}>{terrainApp()}{FEATURES.messaging&&chatOpen&&chatView()}</ErrorBoundary>;
  return (
